...
if(isset($_POST['keydel']) && $_POST['keydel']=='delete')
{
	...
	//print_r($ids);
	//exit();
	$sql = "DELETE FROM catagories WHERE ID IN (".$ids.")";
	mysql_query($sql) or die(mysql_error());

	$msg = '<div class="callout callout-success">
			<h4>Roles has been deleted</h4>
		  </div>';
	
}

$sql = "SELECT c.ID,c.CatagoryName,c.ParentID,c.Image,c.Status,c.Description,c1.CatagoryName as NameOfParent,DATE_FORMAT(c.DateAdded,'%b %d %Y %h:%i %p') as Added,DATE_FORMAT(c.DateModified,'%b %d %Y %h:%i %p') as Updated FROM catagories c LEFT JOIN catagories c1 ON c1.ID=c.ParentID where c.ID <>0";
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <button type="button" style="float:right; margin-right:20px;" onClick="location.href='AddNewCatagory.php'" class="btn-info btn btn-sm">Add</button>
    <!-- Main content -->
    <?php
            ...
            <form action="<?php echo $self; ?>" id="formdel" method="post">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th><?php echo $row['CatagoryName']; ?></th>
                  <th><?php echo $row['ParentID']; ?></th>
                  <th><img style="width:60px;" src="<?php echo DIR_UPLOADING_CATAGORIES.$row['Image'];?>"/></th>
