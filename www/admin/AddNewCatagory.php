...
if(isset($_POST["submit_key"])&& $_POST['submit_key'] == 'submit_value')
{
	...
	if($msg == "")
	{
		$query = "INSERT INTO catagories SET 
				Status = '".(int)$Status."',
		...
		{ $ext=explode('.',$Image);
          ...
          $moved = move_uploaded_file($_FILES['ImageFile']['tmp_name'],DIR_UPLOADING_CATAGORIES.$Image);
			if($moved)
			{
				$sql = "UPDATE catagories SET Image = '".$Image."' WHERE ID = ".$lastid;
...
<head>
  ...
  <title>Categories | Add New Category</title>
