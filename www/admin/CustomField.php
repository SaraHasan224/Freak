...
if(isset($_POST['keydel']) && $_POST['keydel']=='delete')
{
	...
	$ids = implode(",",$ids);
	$sql = "DELETE FROM customfield WHERE ID IN (".$ids.")";
	mysql_query($sql) or die(mysql_error());
	
	$msg = '<div class="callout callout-success">
                <h4>Custom Field has been deleted</h4>
              </div>';
	
}

$sql = "SELECT ID,FieldName,Type,Options,Status,DATE_FORMAT(DateAdded,'%b %d %Y %h:%i %p') as Added ,DATE_FORMAT(DateModified,'%b %d %Y %h:%i %p') as Updated FROM customfield";
...
<head>
  ...
  $(document).ready(function(){
	  $("#btndel").click(function(){
		  ...
		  {
			  alert("Please select custom fields to delete");
		  }
	  });
	  
	  $(".checkUncheckAll").click(function(){

        $(".chkIds").prop("checked",$(".checkUncheckAll").prop("checked"));	  
	  ...
	  $(".chkIds").click(function(){
		  ...
		  if($(".chkIds:checked").length == $(".chkIds").length)
        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Custom Field</h3>
			  <button style="float:right" id="btndel" type="button" class="btn btn-danger btn-sm">Delete</button>
			  <button style="float:right; margin-right:10px;" type="button" onClick="location.href='AddNewCustomField.php'" class="btn btn-sm btn-info">Add</button>
            </div>
			<?php 
			...
			<form action="<?php echo $self;?>" id="formdel" method="post">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><button type="button" onClick="location.href='EditCustomField.php?CustomFieldID=<?php echo $row['ID']; ?>'" class="btn btn-info btn-sm">Edit</button></td>
                </tr>
                <?php
				}
				?>
