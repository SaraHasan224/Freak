...
function get_catagories_menu($ParentID = 0,$SelectedID = 0,$indent = '')
{
	...
	if(mysql_num_rows($r) > 0)
	{
		...
		{
			 echo '<option '.($SelectedID==$RsC['ID'] ? 'selected' : '' ).' value="'.$RsC['ID'].'">'.$indent.'|_'.$RsC['CatagoryName'].$SelectedID.'</option>';
			 get_catagories_menu($RsC['ID'],$SelectedID,$indent . INDENT.'');
			 
		}
	}
}
if(isset($_REQUEST["CatagoryID"]) && ctype_digit($_REQUEST["CatagoryID"]))
       $CatagoryID=trim($_REQUEST["CatagoryID"]);
...
if(isset($_POST['submit_key']) && $_POST['submit_key'] == 'submit_value')
{
	...
	if($msg == "")
	{
		...
		if($Image!='')
		{
			$ext = explode('.',$Image);
			$ext = end($ext);
			$Image = 'C-'.$CatagoryID.'.'.$ext;
			$moved = move_uploaded_file($_FILES['Image']['tmp_name'],DIR_UPLOADING_CATAGORIES.$Image);
			if($moved)
			{
				$sql = "UPDATE catagories SET Image = '".$Image."' WHERE ID = ".$CatagoryID;
				// echo $sql;
				// exit();
				mysql_query($sql) or die(mysql_error());
			}
		...
		{
			 $sql = "SELECT Image FROM catagories WHERE ID ='".$CatagoryID."'";
...
else
{
	...
	$sql="SELECT ID,ParentID,CatagoryName,Image,Description,Status FROM catagories WHERE ID='".$CatagoryID."'";
	$res=mysql_query($sql) or die(mysql_error());
	$num=mysql_num_rows($res);
      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div class="box box-info">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form class="form-horizontal" action="<?php echo $self; ?>?CatagoryID=<?php echo $CatagoryID;?>" method="post" enctype="multipart/form-data">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="inputEmail3" name="CatagoryName" value="<?php echo $CatagoryName; ?>" placeholder="Enter Catagory Name">
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
						<img style="width:150px;" src="<?php echo DIR_UPLOADING_CATAGORIES.$Image?>"/>
                  </div>
                </div>
			    ...
			    <select class="form-control" name="ParentID">
				  ...
				  ?>
                    ...
                    <option <?php echo($ParentID==$row['ID'] ? 'selected' : '' ); ?> value="<?php echo $row['ID']; ?>"><?php echo $row['CatagoryName']; ?>
					</option>
